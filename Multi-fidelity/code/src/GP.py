...
class GP:
    ...
    def neg_likelihood(self, theta):
        ...
        K = RBF(self.train_x, self.train_x, hyp, np.arange(self.dim)) + sn2*np.eye(self.num_train)
    ...
    def predict(self, test_x):
        ...
        hyp = self.theta[1:]
        tmp = RBF(test_x, self.train_x, hyp, np.arange(self.dim))
        py = np.dot(tmp, self.alpha)
        ps2 = sn2 + RBF(test_x, test_x, hyp, np.arange(self.dim)) - np.dot(tmp, chol_inv(self.L, tmp.T))
        return py, ps2
