...
def make_figure(model, dataset, test_x, test_y):
    ...
    plt.figure()
    plt.subplot(211)
    plt.plot(high_x[0], high_y[0], 'ro', markersize=4.5, label='high-fidelity training data')
    plt.plot(test_x[0], test_y[0], 'r-', label='exact high fidelity', linewidth=1.5)
    plt.plot(test_x[0], py2, 'b--', label='prediction', linewidth=1.5)
    plt.fill_between(test_x[0], py2-3*ps22, py2+3*ps22, facecolor='lightgray', alpha=0.5, label='3-standard-deviations band')
    plt.xticks(fontsize=9)
    plt.yticks(fontsize=9)
    plt.ylabel('y', {'size':12})
    leg = plt.legend(frameon=True, fontsize=8, loc='lower right')
    leg.get_frame().set_alpha(1.0)

    plt.subplot(212)
    plt.plot(test_x[0], EI(model, test_x), linewidth=1.5)
    plt.yticks(fontsize=9)
    ...
    plt.figure()
    plt.subplot(211)
    plt.plot(low_x[0], low_y[0], 'mo', markersize=4.5, label='low-fidelity training data')
    plt.plot(test_x[0], low(test_x[0], bounds), 'm--', label='exact low fidelity', linewidth=1.5)
    plt.plot(high_x[0], high_y[0], 'ro', markersize=4.5, label='high-fidelity training data')
    plt.plot(test_x[0], test_y[0], 'r-', label='exact high fidelity', linewidth=1.5)
    plt.plot(test_x[0], py2, 'b--', label='prediction', linewidth=1.5)
    plt.fill_between(test_x[0], py2-3*ps22, py2+3*ps22, facecolor='lightgray', alpha=0.5, label='3-standard-deviations band')
    ...
    plt.plot(high_x[0], high_y[0], 'ro', markersize=4.5, label='high-fidelity training data')
    plt.plot(test_x[0], test_y[0], 'r-', label='exact high fidelity', linewidth=1.5)
    plt.plot(test_x[0], py, 'b--', label='prediction', linewidth=1.5)
    plt.fill_between(test_x[0], py-3*ps, py+3*ps, facecolor='lightgray', alpha=0.5, label='3-standard-deviations band')
    plt.yticks(fontsize=9)
    plt.ylabel('y', {'size':12})
    plt.xticks(fontsize=9)
    plt.xlabel('x', {'size':12})
