...
for i in range(iteration):
    ...
    plt.plot(test_x[0], py[0], 'b-', label='low prediction', linewidth=1)
    plt.plot(test_x[0], low(test_x[0]), 'r-', label='low exact', linewidth=1)
    plt.fill_between(test_x[0], py[0]-3*ps2[0], py[0]+3*ps2[0], facecolor='orange', alpha=0.5, label='low 3 std band')
    ax = plt.gca()
    ax.set_ylim([-20,20])
    ...
    _, tmp = model.predict_low(x)
    idx = np.argsort(tmp[0])[-num_points:]
